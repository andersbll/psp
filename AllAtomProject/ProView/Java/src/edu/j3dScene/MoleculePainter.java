...
public class MoleculePainter {
	...
	private class SceneMolecule{
		...
		private final List<Shape> bondShapes = new ArrayList<Shape>();
		...
		SceneMolecule(List<double[]> coords, List<Integer> atomTypes, boolean dim, boolean cAlpha){
			...
			for(int i=0;i<coords.size();i++){
				...
				Sphere s = new Sphere(new Vector(c[0],c[1],c[2]), atomRadii[t]);
				Color col = colors[t];
	...
	public static void main(String[] args){
		...
		while(true){
			cs2.get(edu.math.Toolbox.randBetween(0,cs2.size()))[edu.math.Toolbox.randBetween(0, 2)] += 
				edu.math.Toolbox.randBetween(-0.04f, 0.04f);
			//coords.get(1)[0]+=0.1;
			mp.updateCoords();
			mp.centerView();
			//mp.autoZoom();
