...
\usepackage[us,danish]{babel} % last language decides document language!
...
In this project we want to develop a model for representing protein structures including all of the atom.
...
structure.
With this method we will investigate whether the
problem gets computationally simpler when the given three-dimensional backbone structure
...
The building stone of proteins are amino acids\cite{branden}. A protein
consist of
at least one linear chain of amino acids. In Figure
\ref{fig:amino_connect} we have shown how a sequence of amino acids
are connected. A carbon atom is located at the center of each amino
...
A realistic energy calculation will require a molecular insight that is beyond the scope of this project.
Therefore, we limit ourselves to a distance calculation between all atom pairs.
<rant om hvorfor distance-målet er ok?>.
