...
%DOCUMENT INFO
\title{
  Prediction of Side-chain Conformations\\ with an All-atom Model \\
...
will use this model to find side-chain conformations of proteins
given only the three-dimensional structures of their backbones.

Using this method we will investigate whether the problem is simpler
in terms of the number of computation steps (e.g. number of collision
detections between atom pairs) when the given backbone structure is
closer to the physically correct structure of the protein.
...
between the individual $C_\alpha$ atoms\cite{lotan04}, \textit{the
  torsion angles}. Thus, the approach taken by the algorithms team at
