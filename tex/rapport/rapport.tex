...
\begin{onecolabstract}
  ...
  possible. On realistic protein backbones the collision avoidance algorithm is able to reduce the number of collisions to 1 every 4700 amino acids in a protein sequence.
  \vspace{1.5cm}
\end{onecolabstract}
]
\tableofcontents*
...
On protein backbones fitted to \Ca-traces by or fitting algorithm, the collision avoidance algorithm is able to reduce the number of collisions to 1 every 500 amino acids.
On realistic protein backbones the collisions are reduced to 1 every 4700 amino acids.
Compared to a state of the art method, our algorithm performs well when measuring the ability to avoid collisions.
In terms of RMSD our algorithm performs worse.
